...
## 305. 下游已有文学职责时直接删除上游重复，不做重复迁移（2026-09-11）

用户追问：从结构 Prompt 删除的文学规则应该在哪里补偿。结论是先确认接收方：LiteraryDesign 已负责人物表达、感官、意象和节奏，Story 已负责具体动作、证据分散、反抽象总结和正文呈现；这些职责不需要迁移或复制。Narrative 只保留会影响结构闭合的因果、Seed 已有盲点/代价、冲突尺度和结局所有权。由此本轮删除调查供述、人物反差和记忆锚点的重复规则，并用职责边界测试确认文学规则没有被重复搬运。

## 306. 默认篇幅不是结构合同（2026-09-12）

- **难点**：固定的默认字数下限会把 Story 的自然节奏变成机械配额；短正文即使完成结构、因果和结局，也可能仅因数量不足被拒绝。
- **判断**：篇幅属于正文表达阶段对情节密度的判断，不属于 Outline 的结构契约。已有 `DRAMATIZE/DEVELOP/COMPRESS` 足以传达展开权重，不需要新增逐场字数规划或复制长度规则。
- **方案**：移除系统默认长度输入、`length_ok` 硬判定和 Release 门禁；保留字符计数用于统计。只有原始请求明确提出最低字数时，Story Validator 才根据原文和计数判断用户要求。
- **验证**：短正文 mock 仍可在 Story 图中被接受，导出保留 `chinese_char_count` 且没有 `length_ok`；Story、Release、Outline、Dynamic、Contract 及全仓回归通过。真实生成长度与文学质量仍需独立样本验证。

## 307. 中篇网文定位与软性篇幅建议（2026-09-12）

- **决策**：正文定位从短篇调整为中篇网文，Story Prompt 给出 `10000` 字以上的建议目标，以支持更完整的关系推进、冲突展开和结局兑现。
- **边界**：建议值不能重新变成系统默认下限。`DRAMATIZE/DEVELOP/COMPRESS` 继续决定场景展开权重，Validator 仅检查原始用户请求明确提出的最低字数；默认未提出时不因篇幅失败。
- **验证**：相关 Prompt 与模块描述已统一，短正文 mock 在结构验证通过时仍为 accepted；全量机械测试用于确认数据流未被长度建议改变。

## 308. 结构约束与正文补充采用分层边界（2026-09-12）
