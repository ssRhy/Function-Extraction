...
class FunctionQualityReview(BaseModel):
    ...
    recommendation: str = Field(description="处置建议：OK / REVISE（修订定义）/ MERGE（与近义函数合并）/ SPLIT（把相反方向拆开）")
...
输出 recommendation：OK（无需处理）/ REVISE（修订定义或抽象化）/ MERGE（与近义函数合并，Separation 维度会另行列出近义组）/ SPLIT（把相反方向拆成多个函数）。

不要修改函数名，不要改 Function 数量，只做判断。请以 JSON（json_object）格式输出，字段与上面 schema 一致：reviews 数组，每项含 function_name / bidirectional_conflation / conflation_reason / genre_surface_binding / binding_reason / granularity / recommendation。"""