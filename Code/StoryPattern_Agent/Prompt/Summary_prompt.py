...
class StoryPatternSummary(BaseModel):
    ...
    core_function_names: list[str] = Field(min_length=4)
...
pattern_name 应简洁、抽象、可查询；abstract_definition 用一句到两句说明核心叙事变化。core_function_names 必须逐字使用输入提供的 Function 名称，并按核心顺序输出。optional_steps、applicability_conditions、counterexamples_limitations 都输出字符串数组；没有内容时返回空数组。
...
{"pattern_name": "模式名", "abstract_definition": "抽象定义", "core_function_names": ["Function 名称"], "optional_steps": [], "applicability_conditions": [], "counterexamples_limitations": [], "ending_spec": {"resolves": "待解决的核心冲突", "must_show": ["必须出现的解决动作"], "final_state": "稳定终态"}}
