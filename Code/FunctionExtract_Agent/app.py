...
def export_node(state: NarrativePipelineState) -> dict:
    """舍弃问题函数、抽象归并、重评最终本体并按 PASS 发布快照。"""
    ...
    final_report_path = os.path.join(out_dir, "evaluation_final.json")
    final_state = dict(state)
    context = state.get("evaluation_context") or {}
    final_state["evaluation_context"] = {
        key: context[key] for key in ("manifest_path",) if context.get(key)
    }
    final_state["evaluation_context"]["report_path"] = final_report_path
