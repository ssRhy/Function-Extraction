...
def _make_occurrence(obs: dict, decision: MatchDecision, candidates: list[dict], category, stage) -> dict:
    ...
    return {
        ...
        "participant_ids": obs.get("participant_ids", []),
        "role_bindings": obs.get("role_bindings", {}),
        "relationship_deltas": obs.get("relationship_deltas", []),
...
def _build_batch_input(batch: list[dict], candidates: list[list[dict]]) -> str:
    ...
    for o, cand in zip(batch, candidates):
        lines.append(json.dumps({
            ...
            "participant_ids": o.get("participant_ids", []),
            "role_bindings": o.get("role_bindings", {}),
            "relationship_deltas": o.get("relationship_deltas", []),
