...
def test_tampering_is_detected(tmp_path):
    path = publish_snapshot(_functions(), _evaluation(), "bootstrap", "test", str(tmp_path))
...
def test_occurrence_tampering_is_detected(tmp_path):
    path = publish_snapshot(
        _functions(), _evaluation(), "evolve", "test", str(tmp_path), _occurrences(),
    )
...
def test_unknown_occurrence_function_rejected(tmp_path):
    ...
    with pytest.raises(ValueError, match="未知 Function"):
        publish_snapshot(
            _functions(), _evaluation(), "evolve", "test", str(tmp_path), [occurrence],
        )


def test_identical_publish_is_idempotent(tmp_path):
    first = publish_snapshot(_functions(), _evaluation(), "evolve", "test", str(tmp_path))
    second = publish_snapshot(_functions(), _evaluation(), "evolve", "test", str(tmp_path))
    assert second == first
...
def test_same_id_with_different_content_is_rejected(tmp_path, monkeypatch):
    ...
    publish_snapshot(_functions(), _evaluation(), "bootstrap", "test", str(tmp_path))
    changed_evaluation = {"verdict": "PASS", "passed_dimensions": ["cohesion"]}
    with pytest.raises(FileExistsError):
        publish_snapshot(_functions(), changed_evaluation, "bootstrap", "test", str(tmp_path))
