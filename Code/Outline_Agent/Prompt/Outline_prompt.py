...
规则：人物数量保持最少但至少足以承接后续 Function 的角色槽位；主人公的 stance_toward_protagonist 填 self，其他人物只能根据核心冲突和初始关系填 support、obstruct、mixed 或 neutral；role 不自动决定立场。关系只写用户要求或核心冲突明确需要的最低事实，不预设信任、爱情、背叛或和解。ending_direction 必须按“可观察解决动作 → 直接冲突结果 → 稳定终态”写成明确的单一结局方向，不能只留下新的危险或悬念。"""


SEED_PROMPT = """你是网文大纲策划。给定一条包含 FunctionContract 的 Function 序列（叙事结构骨架）和模板级 ending_spec，生成故事种子。人物必须覆盖合同中的角色槽位，核心冲突必须能承接合同中的前置条件和状态效果。只输出 JSON，字段名严格如下：
{
  "genre": "题材",
  "world_setting": "世界观",
...
3. role 只表示人物承担的结构位置，不自动规定其开场态度或关系类型。stance_toward_protagonist 必须明确填 self、support、obstruct、mixed 或 neutral，并能由 relationships、goal 和核心冲突解释。题材标签、人物性别、共同行动或“关系发展/稳定”等宽泛描述，都不是亲情、友情、合作、依附、爱情或其他具体关系的充分依据。只有 Function、角色槽位或 ending_spec 明确要求某种关系时才能设定；否则从核心冲突所需的最低关系事实开始，不得预设信任、敌意、忠诚、爱情、原谅、依赖或背叛。
4. ending_direction 必须吸收 ending_spec 的结局要求，按“可观察解决动作 → 直接冲突结果 → 稳定终态”写成明确方向，不能只写新的危险、悬念或后续行动。若结局同时改变生存、身份、权力、资源、责任或人物关系等多个维度，必须分别说明各自如何解决，不能用一个动作自动完成无直接因果关系的全部变化。关系结局的类型和强度不得超过 Function 链能够建立的上界；宽泛的“向好”“稳定”或“修复”必须保守解释为已有关系维度内的改善，不得自行升级关系类型或新增承诺。若 ending_spec 为 null，自行给出一个与核心冲突一致的明确解决方向。"""
...
输入中的 reference_motifs 是所选 Pattern 的局部 motif 证据；每个 chain step 中的 reference_instance_cases 是 Function 的真实实现参考；二者只用于题材化，不改变既定 Function 链。`ending_target` 是实际结局目标：有 Pattern 结局规范时来自该规范，否则来自 seed.core_conflict 和 seed.ending_direction；`ending_spec=null` 不表示缺少结局。`ending_budget` 中的伏笔、义务和关系状态是 ending 可使用的前序证据范围。
规则：
...
其中 final_ledger 必须是字符串数组；每一项都是一条可读的状态记录，不得输出对象、字典、嵌套数组或额外字段。`ending_target` 是必须回应的结局目标；`ending_budget` 是 ending 允许使用的前序证据和关系状态上限。`ending_spec=null` 不表示缺少 ending，实际结局始终写在输出的 `ending` 字段中。
