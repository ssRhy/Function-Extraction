...
其中 final_ledger 必须是字符串数组；每一项都是一条可读的状态记录，不得输出对象、字典、嵌套数组或额外字段。`ending_target` 是必须回应的唯一实际结局目标，来源是本轮 LLM seed，而不是 Pattern 的 `ending_spec`；`ending_budget` 是 ending 允许使用的前序证据和关系状态上限。`ending_spec` 只保留在上游/导出结果中供审计，不把它的具体解决动作、must_show 或 final_state重新变成硬要求。实际结局始终写在输出的 `ending` 字段中。
