...
## 本轮（2026-09-07）：收窄 repairable 定义并重跑两条失败判定

- Validator Prompt 只做一处语义修正：`repairable=false` 仅表示固定输入本身互相矛盾；只要固定 Outline、Function constraints、scene plan 和 ending 一致，正文遗漏结局、主线偏离、因果不足、临时解决方案或长度不足均必须返回 `repairable=true`。
